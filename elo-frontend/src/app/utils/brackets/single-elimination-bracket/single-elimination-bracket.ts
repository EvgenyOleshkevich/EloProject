...
import { buildConnectors, buildMatchLayouts, buildWinnerRounds } from '../../functions';
...
@Component({
  selector: 'single-elimination-bracket',
  standalone: true,
  imports: [CommonModule, DragDropModule, CdkDragPlaceholder],
  templateUrl: './single-elimination-bracket.html',
...
export class SingleEliminationBracket {
  ...
  // @Output() swapPlayers = new EventEmitter<{
  //   from: BracketSlotRef;
  //   to: BracketSlotRef;
  // }>();
  ...
  // winnerRounds = computed(() => {
  //   const tournament = this.tournament();
  //   if (!tournament) {
  //     return [];
  //   }

  //   return buildWinnerRounds(tournament.matches);
  // });
  ...
  connectors = computed(() => buildConnectors(this.matchLayouts()));
  ...
  svgWidth = computed(() => {
    ...
    const rounds = tournament.matches.reduce((max_value, match) => Math.max(max_value, match.tournamentInfo.round), 0);
    return 24 + rounds * (220 + 80);
  });
